...
-(void)showErrorMsg{
    [SVProgressHUD dismissWithError:@"数据请求失败，请检查网络配置!"];
}
...
- (void)showWithStatus:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType networkIndicator:(BOOL)show {
    //设置定时器时间
//    NSTimeInterval time = 32.f;
//	self.fadeOutTimer = [NSTimer scheduledTimerWithTimeInterval:time target:self selector:@selector(showErrorMsg) userInfo:nil repeats:NO];
