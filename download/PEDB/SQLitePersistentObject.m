...
+ (int)execBatchSQL: (NSString *)batch
{
    sqlite3 *database = [[SQLiteInstanceManager sharedManager] database];
    char * errMsg = NULL;
    int ret = sqlite3_exec( database, [batch UTF8String] , NULL, NULL, &errMsg );
    return ret;
}

+ (NSMutableArray *)execArraySQL: (NSString *)sqlStr
{
    ...
    int rows = 0;
    char **dataResult;
    sqlite3_get_table( database , [sqlStr UTF8String] , &dataResult , &rows , NULL , &errMsg );
    for (int i = 1; i <= rows; i++)
    {
        //        printf("%s\n",dataResult[i]);
        NSString *customerPk = [NSString stringWithFormat:@"%s",dataResult[i]];
        //        id customerPk = (id)dataResult[i];
        [array addObject:customerPk];
    }
...
+ (void)clearCache
{
	...
	if(checkedTables != nil){
        ...
        [checkedTables release];
        checkedTables=nil;
        
    }
    
    if (classPropertyTables!=nil) {
        [classPropertyTables removeAllObjects];
        [classPropertyTables  release];
        classPropertyTables = nil;
    }
}
...
/* add by pengjiang 只取名称字段 */
+(NSArray *)findByCriteriaForName:(NSString *)criteriaString
{
	...
	NSString *query = [NSString stringWithFormat:@"SELECT name FROM %@ %@", [[self class] tableName], criteriaString];
	sqlite3_stmt *statement;
			...
			for (i=0; i <  sqlite3_column_count(statement); i++)
			{
				...
				else
				{
					...
					else if ([colType hasPrefix:@"@"])
					{
						...
						{
							const char * rawData = (const char * )sqlite3_column_blob(statement, i);
...
+(NSArray *)pairedArraysForProperties:(NSArray *)theProps withCriteria:(NSString *)criteriaString
{
	...
	[query appendFormat:@" FROM %@ %@ ", [[self class] tableName], criteriaString];
...
+(NSArray *)queryForProperties:(NSArray *)theProps withCriteria:(NSString *)criteriaString
{
	NSMutableArray *ret = [NSMutableArray array];
	[[self class] tableCheck];
	...
	NSMutableString *query = [NSMutableString stringWithString:@"select DISTINCT"];
...
//static NSDictionary* theProps = nil;

NSMutableDictionary* classPropertyTables = nil;

+(NSDictionary *)propertiesWithEncodedTypes
{
    if (classPropertyTables==nil) {
        classPropertyTables = [[NSMutableDictionary alloc]initWithCapacity:8];
    }
    if([[classPropertyTables allKeys]containsObject:self.className]){
        
        return [classPropertyTables objectForKey:self.className];
    }else{
        NSDictionary* dict =  [self propertiesWithEncodedTypesImpl];
        [classPropertyTables setObject:dict forKey:self.className];
        return dict;
    }
}

+(NSDictionary *)propertiesWithEncodedTypesImpl{
    
	// Recurse up the classes, but stop at NSObject. Each class only reports its own properties, not those inherited from its superclass
	NSMutableDictionary *theProps;
	
	if ([self superclass] != [NSObject class])
		theProps = (NSMutableDictionary *)[[self superclass] propertiesWithEncodedTypesImpl];
	else
		theProps = [NSMutableDictionary dictionary];
    //theProps = [[NSMutableDictionary alloc]initWithCapacity:20];
...
+ (BOOL)resolveClassMethod:(SEL)theMethod
{
	@synchronized(self)
	{
		...
		NSString *methodBeingCalled = [[[NSString alloc] initWithUTF8String:methodName]autorelease];
...
+ (NSString *)tableName
{
    ...
    //		tableNamesByClass = [[[NSMutableDictionary alloc] init]autorelease];
    //
